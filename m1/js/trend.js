...
function trend_btn_saveFn(){
	...
	zeai.confirm('确定提交么？<br>以下情形话题将被删除或封号：<br>1．请勿在内容里留下任何形式的联系方法<br>2．请勿发表涉黄涉政言论<br>3．请勿发表不文明或辱骂攻击他人言论',function(){
		zeai.msg("请稍后，正在保存");
...
function btnpicFn(){
	...
	if(is_h5app()){
		app_uploads({url:HOST+'/m1/trend.php?submitok=ajax_photo_up_app',num:1},function(e){
...
*/		
	...
	//agree
	var agreeDIV = dl.getElementsByTagName("div")[0].children;
	var agree = agreeDIV[0],chat = agreeDIV[1],agreehtml = agree.getElementsByTagName("span")[0],bbshtml = chat.getElementsByTagName("span")[0];
		...
		agree.onclick = function (){
			...
			zeai.ajax({url:'m1/trend'+zeai.extname,data:{submitok:'ajax_agree',tid:tid}},function(e){rs=zeai.jsoneval(e);
				if(rs.flag==1){
					agree.className    = 'ed';
					agree.onclick      = function (){}
					agreehtml.html(parseInt(agreehtml.innerHTML)+1);
					var b=agreehtml.parentNode.getElementsByTagName("b")[0];b.show();
					...
					if (rs.num == 1){
						...
						if(zeai.empty(ul)){
							var j  = document.createElement('div');j.className = 'j';dl.appendChild(j);
							dl.appendChild(em);
						}else{
							ul.parentNode.insertBefore(em,ul);
						}
					}else{
						var em  = dl.getElementsByTagName("em")[0];
					}
	...
	chat.onclick = function (){
		if(!zeai.empty(o('trendbbs'))){o('trendbbs').parentNode.remove();return false;}
		if(!zeai.empty(o('trendbbs'))){o('trendbbs').parentNode.remove();}
		var bbs=zeai.addtag('div');bbs.class('trend_bbsadd');
		var textarea=zeai.addtag('textarea');textarea.class('textarea small_big');textarea.id='trendbbs';textarea.setAttribute("placeholder","请文明发言");
		var btn=zeai.addtag('button');btn.class('btn size2 HONG small_big');btn.type='button';btn.html('提交评论');
		bbs.append(textarea);bbs.append(btn);
		dl.insertBefore(bbs,chat.parentNode.nextSibling);
		textarea.onblur=function(){setTimeout(function(){window.scrollTo(0,0)},200);}
		btn.onclick =function (){
			textarea.value = zeai.clearhtml(textarea.value);
			if(zeai.empty(textarea.value)){zeai.msg('请输入内容(200字以内)');return false;}
			var fid=chat.parentNode.getAttribute("tid");
	zeai.confirm('确定提交么？<br>以下情形话题将被删除或封号：<br>1．请勿在内容里留下任何形式的联系方法<br>2．请勿发表涉黄涉政言论<br>3．请勿发表不文明或辱骂攻击他人言论',function(){
			zeai.ajax({url:'m1/trend'+zeai.ajxext+'submitok=trend_bbs_add',data:{fid:fid,content:textarea.value}},function(e){rs=zeai.jsoneval(e);
				zeai.msg(0);zeai.msg(rs.msg);
				if(rs.flag==1){
					bbshtml.html(parseInt(bbshtml.innerHTML)+1);
					var agreenumm=parseInt(agreehtml.innerHTML);
					if(agreenumm==0){var j=zeai.addtag('div');j.class('j');dl.append(j);}
					var ul=dl.getElementsByTagName("ul")[0];
					if(zeai.empty(ul)){ul=zeai.addtag('ul');dl.append(ul);}
					setTimeout(function(){ul.insertAdjacentHTML('afterbegin',html_decode(rs.C));},100);
					bbs.addClass('big_small');setTimeout(function(){bbs.remove();},300);
				}
			});
	});
		}
...
function trendOnscroll(){
	...
	if (H-t-cH <128 && t>100){//t+cH==H
		if (p > totalP){
			...
			var url_str=HOST+'/m1/trend'+zeai.extname+'?t='+timestamp2;	
			//if(!zeai.empty(cs))Object.assign(postjson,tojson(cs.split('&')));
			zeai.ajax({'url':url_str,data:postjson},function(e){
			if (e == 'end'){
...
trendtop.addEventListener('touchmove', function(e){e.preventDefault();});
nav.addEventListener('touchmove', function(e){e.preventDefault();});
function zeaiplay(zeai_cn) {
	if(mobkind()=='android'){
		zeai.ajax({url:'m1/trend.php?submitok=ajax_vdetail&tid='+zeai_cn},function(e){var rs=zeai.jsoneval(e);
			if(rs.flag==1){
				zeai.openurl(HOST+'/m1/video_detail.php?fid='+rs.vid);
			}else{
				zeai.msg(0);zeai.msg(rs.msg);	
			}
		});
	}else{
		var v = o('zeaiVbox'+zeai_cn);zeai.msg('视频加载中...');v.play();
	}
}
