...
$rt = $db->query("SELECT id FROM ".__TBL_ADMIN__." WHERE id=".$uu." AND password='$pp'");if (!$db->num_rows($rt))exit;
...
if ($j == 'GYL_supdes__www.zeai_cn'){
	...
	//
	$db->query("UPDATE ".__TBL_TG_USER__." SET photo_s='$dbpicname' WHERE id=".$uid);
}else{exit;}
