...
function Zeai_map_save(){
	if(zeai.empty(o(lng).value)){
		...
		zeai.ajax({url:PCHOST+'/my_map'+zeai.ajxext+'submitok=ajax_mapset',form:www_zeai_cn__RORM},function(e){rs=zeai.jsoneval(e);
			if(rs.flag==1){
				var jumpurl=parent.o('jumpurl').value;
				if(!zeai.empty(jumpurl)){
					parent.zeai.openurl(jumpurl);
					//setTimeout(function(){parent.zeai.openurl(jumpurl);},1000);
				}else{
					parent.zeai.msg(rs.msg);parent.zeaimap.click();
				}
			}
		});
	}
