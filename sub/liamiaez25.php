...
function sendemail($email,$title,$content){
	global $_SMS;
	if (ifemail($email)){
		$smtpserver = $_SMS['email_smtp'];//SMTP服务器
		$smtpserverport =25;//SMTP服务器端口
		$smtpusermail = $_SMS['email_uid'];//SMTP服务器的用户邮箱
		$smtpemailto  = $email;//发送给谁
		$smtpuser  = $smtpusermail;//SMTP服务器的用户帐号，注：部分邮箱只需@前面的用户名
		$smtppass  = $_SMS['email_pwd'];//SMTP服务器的用户密码
