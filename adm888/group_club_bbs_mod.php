...
if ( !strip_tags("/^[0-9]{1,9}$/",$classid) )callmsg("Forbidden!","-1");
if ($submitok == "modupdate") {
	if (strlen($content)>30000 || strlen($content)<1) {
		echo '<center><br><br><br><br>内容过多或过少，请控制在1~20000字节以内！<br><br><a href=group_wz_bbs_mod.php?classid='.$classid.'><b>返 回</b></a></center>';
		exit;
	}
	$content = dataIO($content,'in');
	$db->query("UPDATE ".__TBL_GROUP_CLUB_BBS__." SET content='$content' WHERE id='$classid'");
	
}
$rt = $db->query("SELECT content FROM ".__TBL_GROUP_CLUB_BBS__." WHERE id=".$classid);
