...
switch ($submitok){
	case "ajax_addupdate":
		$row = $db->NUM('zeai.area',"title","id=".$id." AND fid=".$fid,__TBL_AREA3__);
		if ($row)json_exit(array('flag'=>0,'msg'=>'地区【'.dataIO($row[0],'out').'】　ID：<font class=Cfc0>'.$id.'</font>　出现重复，请重试','focus'=>'id'));
		$db->query("INSERT INTO ".__TBL_AREA3__." (id,title,fid,px) VALUES ($id,'$title',$fid,".ADDTIME.")");
		json_exit(array('flag'=>1));
	break;
	case "ajax_modupdate":
		$db->query("UPDATE ".__TBL_AREA3__." SET title='$title' WHERE id=".$id);
		json_exit(array('flag'=>1));
	break;
	case "ajax_delupdate":
		if(!ifint($id))json_exit(array('flag'=>0,'msg'=>'forbidden'));
		if ($db->COUNT(__TBL_AREA3__,"fid=".$fid) <= 1)json_exit(array('flag'=>0,'msg'=>'亲，不能删光啊，至少要留一个啊'));
		$db->query("DELETE FROM ".__TBL_AREA3__." WHERE id=".$id);
		json_exit(array('flag'=>1,'msg'=>'删除成功'));
	break;
	case "ding":
		if (!ifint($id))alert_adm_parent('forbidden','back');
		$db->query("UPDATE ".__TBL_AREA3__." SET px=".ADDTIME." WHERE id=".$id);
		header("Location: $SELF?fid=$fid");
	break;
	case "sort_1"://一级上移
		if (!ifint($id))alert_adm_parent('forbidden','back');
		$row = $db->NUM('zeai.area',"px","id=".$id,__TBL_AREA3__);
		if ($row){$px=$row[0];}else{exit;}
		$row = $db->NUM('zeai.area',"id,px","px>".$px." AND fid=".$fid." ORDER BY px LIMIT 1",__TBL_AREA3__);
		if ($row){
			$tmp_id = $row[0];
			$tmp_px = $row[1];
			$db->query("UPDATE ".__TBL_AREA3__." SET px=".$tmp_px." WHERE id=".$id);
			$db->query("UPDATE ".__TBL_AREA3__." SET px=".$px." WHERE id=".$tmp_id);
		}
		header("Location: $SELF?fid=$fid");
	break;
	case "sort_0"://一级下移
		...
		$row = $db->NUM('zeai.area',"px","id=".$id,__TBL_AREA3__);
		if ($row){$px=$row[0];}else{exit;}
		$row = $db->NUM('zeai.area',"id,px","px<".$px." AND fid=".$fid." ORDER BY px DESC LIMIT 1",__TBL_AREA3__);
		if ($row){
			$tmp_id = $row[0];
			$tmp_px = $row[1];
			$db->query("UPDATE ".__TBL_AREA3__." SET px=".$tmp_px." WHERE id=".$id);
...
$row2   = $db->NUM('zeai.area',"fid,title","id=".$fid,__TBL_AREA2__);
$id1    = $row2[0];
$title2 = dataIO($row2[1],'out');

$row = $db->NUM('zeai.area',"title","id=".$id1,__TBL_AREA1__);
...
<?php }else{?>
	<div class="navbox">
		<a href="area1.php">地区管理</a>
		<a href="area2.php?fid=<?php echo $id1;?>">【<?php echo $title1;?>】</a>
		<a class="ed" href="#top">【<?php echo $title2;?>】下属三级地区管理<b><?php echo $db->COUNT(__TBL_AREA3__,"fid=".$fid);?></b></a>
	</div>
	<div class="fixedblank"></div>
	<?php
	$rt = $db->query("SELECT id,title FROM ".__TBL_AREA3__." WHERE fid=".$fid." ORDER BY px DESC");
	$total = $db->num_rows($rt);
	if ($total <= 0 ) {
		if ($submitok !== "add")echo "<div class='nodatatips'>... 暂无内容 ...　　　<a class='btn size2 HUANG' onClick=\"zeai.iframe('新增【".$title1."】-【".$title2."】三级地区','".$SELF."?submitok=add&fid=".$fid."',550,300)\">新增【".$title1."】三级地区</a></div>";
	} else {
		$page_skin = 2;$pagesize=20;$pagemode=3;require_once ZEAI.'sub/page.php';
	?>
	<table class="table0 W98_ Mbottom10 Mtop10">
	  <tr>
		<td width="12%" align="left"><button type="button" class="btn" onClick="zeai.iframe('新增【<?php echo $title1;?>】-【<?php echo $title2;?>】三级地区','<?php echo $SELF;?>?submitok=add&fid=<?php echo $fid;?>',550,300)">新增【<?php echo $title1;?>】-【<?php echo $title2;?>】三级地区</button></td>
	...
	<tr>
		...
		<th width="200">三级地区名称</th>
		<th width="20">&nbsp;</th>
		<th>下属四级地区（点击地区进入管理）</th>
	<th width="20">&nbsp;</th>
		<th width="30">删除</th>
	</tr>
	<?php
	...
	<tr>
		...
		<td>
		<?php
			$rt2=$db->query("SELECT title FROM ".__TBL_AREA4__." WHERE fid=".$id." ORDER BY px DESC");
			...
			if ($total2 > 0) {
				for($i2=1;$i2<=$total2;$i2++) {
					...
					if(!$rows2) break;
					$title2 = $rows2[0];
					echo '<a class="li" href="area4.php?fid='.$id.'" title="管理四级">'.$title2.'</a>';
				}
			}
//			else{
//				echo "<a class='btn size1' onClick=\"zeai.iframe('新增【".$title."】四级地区','area4.php?submitok=add&fid=".$id."',550,300)\">新增【".$title."】四级地区</a>";	
//			}
			?>
            <a class='btn size1' href="area4.php?fid=<?php echo $id;?>" >管理四级</a>
...
<?php if ($submitok == "add") {?>
	save.onclick = function(){
		zeai.ajax({url:'area3'+zeai.extname,form:ZEAIFORM},function(e){var rs=zeai.jsoneval(e);
			if (rs.flag == 1){
...
<?php }else{ ?>
	zeai.listEach('.modbtn',function(obj){
		...
		obj.onclick = function(){
			var V = C.value;
			zeai.ajax('area3'+zeai.ajxext+'submitok=ajax_modupdate&id='+id+'&title='+V+'&fid=<?php echo $fid;?>',function(e){var rs=zeai.jsoneval(e);
	...
	zeai.listEach('.del',function(obj){
		...
		obj.onclick = function(){
			zeai.confirm('您真的要删除【'+title+'】么？',function(){
				zeai.ajax('area3'+zeai.ajxext+'submitok=ajax_delupdate&id='+id+'&fid=<?php echo $fid;?>',function(e){var rs=zeai.jsoneval(e);
					if (rs.flag == 1){location.reload(true);}else{zeai.msg(rs.msg);}
				});
			});
		}
