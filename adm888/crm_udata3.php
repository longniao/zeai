...
if(!in_array('crm',$QXARR))exit(noauth('只有CRM超级管理员才可以管理'));
...
/*		$fieldname=trimhtml($fieldname);$title=trimhtml($title);
		...
		//
		$db->query("INSERT INTO ".__TBL_UDATA__." (fieldname,title,subjsonstr,kind,subkind) VALUES ('$fieldname','$title','$jsonstr',4,$subkind)");
		AddLog('增加【资料属性】->CRM资料字段【'.$title.'】，字段变量：'.$fieldname);
...
*/		$db->query("UPDATE ".__TBL_UDATA__." SET fieldname='".$fieldname."',title='".$title."',subkind=".$subkind.",subjsonstr='".$jsonstr."' WHERE kind=4 AND id=".$id);
		AddLog('修改【资料属性】->CRM资料字段【'.$title.'】，字段变量：'.$fieldname);
...
/*		if(!ifint($id))exit(JSON_ERROR);
		...
		$db->query("DELETE FROM ".__TBL_UDATA__." WHERE kind=4 AND id=".$id);
		@$db->query("ALTER TABLE ".__TBL_USER__." DROP `".$fieldname."`;");
		//callmsg('删除成功',SELF);
		AddLog('删除【资料属性】->CRM资料字段【'.$title.'】，字段变量：'.$fieldname);
...
*/	break;
	case "ajax_pxupdate":
		...
		AddLog('修改排序【资料属性】->CRM资料字段【'.$title.'】，字段变量：'.$fieldname.'，新排序值：'.$px);
		//
...
/*		if(!ifint($id))exit(JSON_ERROR);
		...
		AddLog('修改显示状态【资料属性】->CRM资料字段【'.$title.'】，字段变量：'.$fieldname.'，新状态：'.$flag_str);
		//
		exit(json_encode(array('flag'=>1)));
*/	break;
}
...
<table class="table W90_ Mtop10 Mbottom50">
  <form action="<?php echo SELF;?>" name="ZEAIFORM" id="ZEAIFORM" method="post">
    <tr>
      <td class="tdL">变量名</td>
      <td class="tdR"><input name="fieldname" type="text" class="W150 size2" id="fieldname" size="30" maxlength="20"></td>
    </tr>
    <tr>
      <td class="tdL">字段名称</td>
      <td class="tdR"><input name="title" type="text" class="W150 size2" id="title" size="30" maxlength="20"><span class="tips">中文名称，显示用</span></td>
    </tr>
  <tr>
      <td class="tdL">子选项显示类型</td>
      <td class="tdR">
<input type="radio" name="subkind" id="subkind1" class="radioskin" value="1"><label style="display:none" for="subkind1" class="radioskin-label"><i class="i1"></i><b class="W50 S14">文本</b></label>
...
<input type="radio" name="subkind" id="subkind3" class="radioskin" value="3"><label style="display:none" for="subkind3" class="radioskin-label"><i class="i1"></i><b class="W150 S14">多选 <font class="S12 C999">(会员最多选<script>document.write(checkboxMaxNum);</script>项)</font></b></label>
    </td>
    </tr>
    <tr id="subbox">
      <td class="tdL">子选项</td>
...
<table class="table W90_ Mtop10 Mbottom50">
    ...
    <input type="hidden" name="title" id="title" value="<?php echo $title;?>">
      ...
      <tr>
          <td class="tdL">子选项显示类型</td>
          <td class="tdR">
    <input type="radio" name="subkind" id="subkind1" class="radioskin" value="1"<?php echo ($subkind == 1)?' checked':'';?>><label style="display:none" for="subkind1" class="radioskin-label"><i class="i1"></i><b class="W50 S14">文本</b></label>
    ...
    <input type="radio" name="subkind" id="subkind3" class="radioskin" value="3"<?php echo ($subkind == 3)?' checked':'';?>><label style="display:none" for="subkind3" class="radioskin-label"><i class="i1"></i><b class="W150 S14">多选</b></label>
        </td>
        </tr>
...
<div class="navbox">
<a href="crm_udata3.php" class="ed">CRM资料属性</a>

<div class="clear"></div></div><div class="fixedblank"></div>
<?php
$rt = $db->query("SELECT id,fieldname,title,subjsonstr,flag,px,subkind FROM ".__TBL_UDATA__." WHERE kind=4 ORDER BY px DESC,id");
$total = $db->num_rows($rt);
if ($total <= 0 ) {
	if ($submitok !== "add")echo "<div class='nodatatips'>... 暂无内容 ...</div>";//<a class='btn size2 HUANG' onClick=\"zeai.iframe('新增会员资料字段','".SELF."?submitok=add',550,500)\">新增字段</a>
} else {    
	$page_skin = 2;$pagesize=20;require_once ZEAI.'sub/page.php';
?>
<table class="table0 W98_ Mbottom10 Mtop10">
  <tr>
    <td align="left"> <img src="images/!.png" width="14" height="14" valign="middle"> <font style="vertical-align:middle;color:#999">新增或修改后请及时更新【数据缓存】后才能生效<img src="images/d2.gif"></font>
    <button type="button" class="btn" id="cache"><i class="ico">&#xe642;</i> 更新高速缓存</button>
    </td>
  </tr>
</table>
...
<tr>
	...
	<th width="120">变量名</th>
<th width="80">类型</th>
<th width="120" align="center" class="Pright10">字段名称</th>
	<th>子选项</th>
...
<td width="120" align="center" class="Pright10 S14"><?php echo $title;?></td>
	...
	save.onclick = function(){
		...
		if (subkind2.checked || subkind3.checked){
			...
			zeai.listEach('.tr',function(obj){
				...
				if (!zeai.ifint(subid.value) && subid.value!=0){parent.zeai.msg('请输入ID数字',subid);zeaiifbreak=true;return false;}
		...
		//POST
		zeai.ajax({url:'crm_udata3.php',data:postjson},function(e){var rs=zeai.jsoneval(e);
			if (rs.flag == 1){
...
<?php }else{ ?>
	zeai.listEach('.pxbtn',function(obj){
		...
		obj.onclick = function(){
			var pxV = pxobj.value;
			zeai.ajax('crm_udata3'+zeai.ajxext+'submitok=ajax_pxupdate&id='+id+'&px='+pxV,function(e){var rs=zeai.jsoneval(e);
	...
	zeai.listEach('.switch-label',function(obj){
		...
		obj.onclick = function(){
			...
			zeai.ajax('crm_udata3'+zeai.ajxext+'submitok=ajax_flagupdate&id='+id+'&flag='+flag);
		}
	});
	zeai.listEach('.delico',function(obj){
		...
		obj.onclick = function(){
			zeai.confirm('真的要删除么？（删除后不可恢复，建议先隐藏，需要时再开）',function(){
				zeai.ajax('crm_udata3'+zeai.ajxext+'submitok=ajax_delupdate&id='+id,function(e){var rs=zeai.jsoneval(e);
					if (rs.flag == 1){location.reload(true);}else{zeai.msg(rs.msg);}
				});
			});
		}
	...
	if(o('cache'))cache.onclick = function(){
