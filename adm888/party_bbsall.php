...
switch ($submitok) {
	case"alldel":
		...
		if(count($list)>=1){
			foreach($list as $v){
				$id=intval($v);
				$row = $db->ROW(__TBL_PARTY_BBS__,"fid,content","id=".$id,"num");
				if ($row){
					$fid=$row[0];$content=$row[1];
					$db->query("DELETE FROM ".__TBL_PARTY_BBS__." WHERE id=".$id);
					$db->query("UPDATE ".__TBL_PARTY__." SET bbsnum=bbsnum-1 WHERE bbsnum>0 AND id=".$fid);
					AddLog('【交友活动】->删除活动评论【活动id:'.$fid.'】->评论内容：'.$content);
				}
			}
		}
	...
	case"mod_update":
		...
		AddLog('【交友活动】->修改活动评论->内容：'.$content);
		alert_adm("修改成功","party_bbsall.php?submitok=mod&id=".$id);
	break;
	case"mod":
...
<?php exit;}?>
<div class="navbox">
    <a href="party.php">活动管理</a>
    <a href="party_bbsall.php" class="ed">活动评论管理<?php echo '<b>'.$db->COUNT(__TBL_PARTY_BBS__).'</b>';?></a>
...
if (!empty($Skeyword))$SQL = " AND a.content LIKE '%".dataIO($Skeyword,'in')."%' ";
if (ifint($kuid))$SQL .= " AND b.id =".$kuid;
$rt = $db->query("SELECT a.id,a.fid,a.uid,a.content,a.addtime,b.uname,b.nickname,b.grade,b.sex,b.photo_s,b.photo_f,c.title FROM ".__TBL_PARTY_BBS__." a,".__TBL_USER__." b,".__TBL_PARTY__." c WHERE a.fid=c.id AND a.uid=b.id ".$SQL." ORDER BY a.id DESC LIMIT ".$_ADM['admLimit']);
...
?>
    ...
    <th width="200">来自活动</th>
	<th align="left" >评论内容</th>
	<th width="100" align="center" >发表时间</th>
    ...
    <tr id="tr<?php echo $id;?>">
      <td width="20" height="40"><input type="checkbox" name="list[]" value="<?php echo $id; ?>" uid="<?php echo $uid; ?>" id="id<?php echo $id; ?>" class="checkskin"><label for="id<?php echo $id; ?>" class="checkskin-label"><i class="i1"></i></label></td>
  ...
  if(!empty($rows['nickname']))echo $nickname;?>
    ...
    <td width="200" align="left"  class="S16"><a href="<?php echo Href('party',$fid); ?>" target="_blank" class="aHUI"><?php echo $title; ?></a></td>
    ...
    <td width="80" align="center">
      <a class="edit tips" tips-title='修改' tips-direction='left' clsid="<?php echo $id; ?>" nickname="<?php echo urlencode($nickname);?>">✎</a>
    </td>
    </tr>
	<?php } ?>
    <tfoot><tr>
...
o('btndellist').onclick = function() {
	allList({
		btnobj:this,
		url:'party_bbsall'+zeai.ajxext+'submitok=alldel',
		title:'批量删除',
		msg:'批量删除中...',
		ifjson:true,
		ifconfirm:true
...
zeai.listEach('.edit',function(obj){
	...
	obj.onclick = function(){
		zeai.iframe('修改【'+decodeURIComponent(nickname)+'】评论内容','party_bbsall.php?submitok=mod&id='+id,600,300);
	}
});
</script>
