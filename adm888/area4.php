...
switch ($submitok){
	case "ajax_addupdate":
		$db->query("INSERT INTO ".__TBL_AREA4__." (title,fid,px) VALUES ('$title',$fid,".ADDTIME.")");
		json_exit(array('flag'=>1));
	break;
	case "ajax_modupdate":
		$db->query("UPDATE ".__TBL_AREA4__." SET title='$title' WHERE id=".$id);
		json_exit(array('flag'=>1));
	break;
	case "ajax_delupdate":
		if(!ifint($id))json_exit(array('flag'=>0,'msg'=>'forbidden'));
		//if ($db->COUNT(__TBL_AREA4__,"fid=".$fid) <= 1)json_exit(array('flag'=>0,'msg'=>'亲，不能删光啊，至少要留一个啊'));
		$db->query("DELETE FROM ".__TBL_AREA4__." WHERE id=".$id);
		json_exit(array('flag'=>1,'msg'=>'删除成功'));
	break;
	case "ding":
		if (!ifint($id))alert_adm_parent('forbidden','back');
		$db->query("UPDATE ".__TBL_AREA4__." SET px=".ADDTIME." WHERE id=".$id);
		header("Location: ".SELF."?fid=".$fid);
	break;
	case "sort_1"://一级上移
		if (!ifint($id))alert_adm_parent('forbidden','back');
		$row = $db->ROW(__TBL_AREA4__,"px","id=".$id,"num");
		if ($row){$px=$row[0];}else{exit;}
		$row = $db->ROW(__TBL_AREA4__,"id,px","px>".$px." AND fid=".$fid." ORDER BY px LIMIT 1","num");
		if ($row){
			$tmp_id = $row[0];
			$tmp_px = $row[1];
			$db->query("UPDATE ".__TBL_AREA4__." SET px=".$tmp_px." WHERE id=".$id);
			$db->query("UPDATE ".__TBL_AREA4__." SET px=".$px." WHERE id=".$tmp_id);
		}
		header("Location: ".SELF."?fid=".$fid);
	break;
	case "sort_0"://一级下移
		...
		$row = $db->ROW(__TBL_AREA4__,"px","id=".$id,"num");
		if ($row){$px=$row[0];}else{exit;}
		$row = $db->ROW(__TBL_AREA4__,"id,px","px<".$px." AND fid=".$fid." ORDER BY px DESC LIMIT 1","num");
		if ($row){
			$tmp_id = $row[0];
			$tmp_px = $row[1];
			$db->query("UPDATE ".__TBL_AREA4__." SET px=".$tmp_px." WHERE id=".$id);
...
$row3   = $db->ROW(__TBL_AREA3__,"fid,title","id=".$fid,"num");
$id2    = $row3[0];
$title3 = dataIO($row3[1],'out');
//
$row2   = $db->ROW(__TBL_AREA2__,"fid,title","id=".$id2,"num");
$id1    = $row2[0];
$title2 = dataIO($row2[1],'out');
//
$row1   = $db->ROW(__TBL_AREA1__,"title","id=".$id1,"num");
...
<?php }else{?>
	<div class="navbox">
        ...
        <a href="area3.php?fid=<?php echo $id2;?>">【<?php echo $title2;?>】</a>
		<a class="ed" href="#top">【<?php echo $title3;?>】下属四级地区管理<b><?php echo $db->COUNT(__TBL_AREA4__,"fid=".$fid);?></b></a>
	</div>
	<div class="fixedblank"></div>
	<?php
	$rt = $db->query("SELECT id,title FROM ".__TBL_AREA4__." WHERE fid=".$fid." ORDER BY px DESC");
...
<?php if ($submitok == "add") {?>
	save.onclick = function(){
		zeai.ajax({url:'area4'+zeai.extname,form:ZEAIFORM},function(e){var rs=zeai.jsoneval(e);
			if (rs.flag == 1){
...
<?php }else{ ?>
	zeai.listEach('.modbtn',function(obj){
		...
		obj.onclick = function(){
			var V = C.value;
			zeai.ajax('area4'+zeai.ajxext+'submitok=ajax_modupdate&id='+id+'&title='+V+'&fid=<?php echo $fid;?>',function(e){var rs=zeai.jsoneval(e);
	...
	zeai.listEach('.del',function(obj){
		...
		obj.onclick = function(){
			zeai.confirm('您真的要删除【'+title+'】么？',function(){
				zeai.ajax('area4'+zeai.ajxext+'submitok=ajax_delupdate&id='+id+'&fid=<?php echo $fid;?>',function(e){var rs=zeai.jsoneval(e);
					if (rs.flag == 1){location.reload(true);}else{zeai.msg(rs.msg);}
				});
			});
		}
