...
switch ($submitok){
	case "ajax_addupdate":
		$row = $db->NUM('zeai.area',"title","id='$id'",__TBL_AREA1__);
		if ($row)json_exit(array('flag'=>0,'msg'=>'地区【'.dataIO($row[0],'out').'】　ID：<font class=Cfc0>'.$id.'</font>　出现重复，请重试','focus'=>'id'));
		$db->query("INSERT INTO ".__TBL_AREA1__." (id,title,px) VALUES ($id,'$title',".ADDTIME.")");
	...
	case "ajax_modupdate":
		...
		$oldid = intval($oldid);
		if ($id != $oldid){
			if ($db->NUM('zeai.area',"id","id=".$id." AND id<>".$oldid,__TBL_AREA1__)){
		...
		$db->query("UPDATE ".__TBL_AREA1__." SET title='$title'".$SQL." WHERE id=".$classid);
		*/
		$db->query("UPDATE ".__TBL_AREA1__." SET title='$title' WHERE id=".$id);
		json_exit(array('flag'=>1));
	break;
	case "ajax_delupdate1":
		if(!ifint($id))json_exit(array('flag'=>0,'msg'=>'forbidden'));
		if ($db->COUNT(__TBL_AREA1__) <= 1)json_exit(array('flag'=>0,'msg'=>'亲，不能删光啊，至少要留一个啊'));	
		$rt=$db->query("SELECT id FROM ".__TBL_AREA2__." WHERE fid=".$id);
		...
		if ($total > 0) {
			for($i=1;$i<=$total;$i++) {
				...
				if(!$rows)break;
				$id2 = $rows[0];
				$db->query("DELETE FROM ".__TBL_AREA3__." WHERE fid=".$id2);
			}
		}
		$db->query("DELETE FROM ".__TBL_AREA2__." WHERE fid=".$id);
		$db->query("DELETE FROM ".__TBL_AREA1__." WHERE id=".$id);
		json_exit(array('flag'=>1,'msg'=>'删除成功'));
	break;
	case "ding":
		if (!ifint($id))alert_adm_parent('forbidden','back');
		$db->query("UPDATE ".__TBL_AREA1__." SET px=".ADDTIME." WHERE id=".$id);
		header("Location: $SELF");
	break;
	case "sort_1"://一级上移
		if (!ifint($id))alert_adm_parent('forbidden','back');
		$row = $db->NUM('zeai.area',"px","id=".$id,__TBL_AREA1__);
		if ($row){$px=$row[0];}else{exit;}
		$row = $db->NUM('zeai.area',"id,px","px>".$px." ORDER BY px LIMIT 1",__TBL_AREA1__);
		if ($row){
			$tmp_id = $row[0];
			$tmp_px = $row[1];
			$db->query("UPDATE ".__TBL_AREA1__." SET px=".$tmp_px." WHERE id=".$id);
			$db->query("UPDATE ".__TBL_AREA1__." SET px=".$px." WHERE id=".$tmp_id);
		}
		header("Location: $SELF");
	break;
	case "sort_0"://一级下移
		...
		$row = $db->NUM('zeai.area',"px","id=".$id,__TBL_AREA1__);
		if ($row){$px=$row[0];}else{exit;}
		$row = $db->NUM('zeai.area',"id,px","px<".$px." ORDER BY px DESC LIMIT 1",__TBL_AREA1__);
		if ($row){
			$tmp_id = $row[0];
			$tmp_px = $row[1];
			$db->query("UPDATE ".__TBL_AREA1__." SET px=".$tmp_px." WHERE id=".$id);
...
<?php }else{?>
	<div class="navbox">
	<a class="ed" href="#top">工作地区管理<b><?php echo $db->COUNT(__TBL_AREA1__);?></b></a>
	</div>
	...
	<tr>
		...
		<td>
			<?php
			$rt2=$db->query("SELECT id,title FROM ".__TBL_AREA2__." WHERE fid=".$id." ORDER BY px DESC");
			...
			if ($total2 > 0) {
				for($i2=1;$i2<=$total2;$i2++) {
					...
					$id2    = $rows2[0];
					$title2 = $rows2[1];
					echo '<a class="li" href="area2.php?fid='.$id.'" title="管理二级">'.$title2.'</a>';
				}
			}
			echo '<a class="btn size1" href="area2.php?fid='.$id.'" title="管理二级">管理二级</a>';
			?>
		</td>
		<td width="20">&nbsp;</td>
...
<?php if ($submitok == "add") {?>
	save.onclick = function(){
		zeai.ajax({url:'area1'+zeai.extname,form:ZEAIFORM},function(e){var rs=zeai.jsoneval(e);
			if (rs.flag == 1){
...
<?php }else{ ?>
	zeai.listEach('.modbtn',function(obj){
		...
		obj.onclick = function(){
			var V = C.value;
			zeai.ajax('area1'+zeai.ajxext+'submitok=ajax_modupdate&id='+id+'&title='+V,function(e){var rs=zeai.jsoneval(e);
	...
	zeai.listEach('.del',function(obj){
		...
		obj.onclick = function(){
			zeai.confirm('将同步删除【'+title+'】下属二级和三级地区，真的要删除么？',function(){
				zeai.ajax('area1'+zeai.ajxext+'submitok=ajax_delupdate1&id='+id,function(e){var rs=zeai.jsoneval(e);
					if (rs.flag == 1){location.reload(true);}else{zeai.msg(rs.msg);}
				});
			});
		}
