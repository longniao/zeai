...
if ($submitok == 'ajax_qianxian_add' || $submitok == 'ajax_qianxian_chk'){
	if ( !ifint($senduid))json_exit(array('flag'=>0,'msg'=>'客户UID不正确'));
	if ( !ifint($uid))json_exit(array('flag'=>0,'msg'=>'被牵线人UID不正确'));
}
if ($submitok == 'ajax_qianxian_add'){
	$row2 = $db->ROW(__TBL_USER__,"nickname,agentid,areaid","id=".$senduid,'num');$sendnickname= $row2[0];$sendagentid= $row2[1];$sendareaid= $row2[2];
	$row2 = $db->ROW(__TBL_USER__,"nickname,agentid,areaid","id=".$uid,'num');$nickname= $row2[0];$agentid= $row2[1];$areaid= $row2[2];
...
/**************************/
$SQL  = "   (flag=1 OR flag=-2) AND kind<>4 AND crm_flag<3 ";
$SQL .= " AND id<>".$uid;

if($session_kind == 'crm'){
	if(!in_array('crm_user_select',$QXARR))exit(noauth());
	if($hyk!=3)$SQL .= getAgentSQL();//整站客户库
}else{
...
$Uif2      = $row['if2'];
$Usjtime   = $row['sjtime'];
$Ucrm_ugrade = $row['crm_ugrade'];

$Usex_str   = udata('sex',$Usex);
...
if($k=='qx'){
	...
	$qxneednum=$Ucrm_qxnum;
	$qxnum = $db->COUNT(__TBL_QIANXIAN__,"flag=2 AND senduid=".$uid);//成功
	$rowqx = $db->ROW(__TBL_QIANXIAN__,"flag","senduid=".$uid." ORDER BY id DESC LIMIT 1",'name');
	$qxflag_str= crm_arr_title($qxflagARR,$rowqx['flag']);
	
	$yjneednum=$Ucrm_yjnum;
	$yjnum = $db->COUNT(__TBL_CRM_MATCH__,"meet_flag=3 AND uid=".$uid);//完成
	$rowyj = $db->ROW(__TBL_CRM_MATCH__,"meet_flag","uid=".$uid." ORDER BY px DESC,id DESC LIMIT 1");
	$yjflag_str= crm_arr_title($meet_flagARR,$rowyj['meet_flag']);
}
...
<?php if ($t == 3){
	...
	.crm_photo_m .gradeico{position:absolute;left:-5px;top:15px}
    ...
    <table class="table0" style="border-bottom:#eee 1px solid">
      <tr>
        <td width="200" align="left" valign="top"><em class="crm_photo_m" ><?php echo $photo_m_str; ?><div class="gradeico"><?php echo crm_ugrade_time($uid,$Ucrm_ugrade,'btn_djs_noA');?></div></em></td>
        ...
        <div class="zoyqbox">
            ...
            <?php }elseif($k == 'yj'){ ?>
                <font class="C999">需约见：</font><b><?php echo $yjneednum;?></b>次<br>
                <font class="C999">已完成：</font><b><?php echo $yjnum1;?></b>次<br>
                <font class="C999">进行中：</font><b><?php echo $yjnum2;?></b>次<br>
                <font class="C999">暂停中：</font><b><?php echo $yjnum3;?></b>次<br>
                <font class="C999">最近状态：</font><?php echo $yjflag_str;?>
            <?php }else{ ?>
                <font class="C999">需牵线：</font><b><?php echo $qxneednum;?></b>次<br>
                <font class="C999">成功了：</font><b><?php echo $qxnum;?></b>次<br>
                <font class="C999">最近状态：</font><?php echo $qxflag_str;?>
                <font class="C999">需约见：</font><b><?php echo $yjneednum;?></b>次<br>
                <font class="C999">已完成：</font><b><?php echo $yjnum;?></b>次<br>
                <font class="C999">最近状态：</font><?php echo $yjflag_str;?>
            <?php }?>
        </div>
        </td>
        ...
        <div class="zoyqbox">
            ...
            <font class="C999">性别：</font><?php echo $Usex_str;?><br>
    ...
    <table class="table0" style="margin-top:0">
        ...
        <td height="50" align="left" class="border0 S14" style="min-width:980px">
            <form id="zeaiFORMso" method="get" action="<?php echo SELF; ?>">
                配对范围：
                <input type="radio" name="hyk" id="hyk1" class="radioskin" value="1"<?php echo ($hyk == 1)?' checked':'';?>><label for="hyk1" class="radioskin-label"><i class="i1"></i><b class="W80 S14">我的客户</b></label>
                <input type="radio" name="hyk" id="hyk2" class="radioskin" value="2"<?php echo ($hyk == 2 || empty($hyk))?' checked':'';?>><label for="hyk2" class="radioskin-label"><i class="i1"></i><b class="W80 S14">我的门店</b></label>
                ...
                <button type="submit" class="btn HONG2 size3 "><i class="ico">&#xe6c4;</i> 开始配对</button>
            </form>
        </td>
        </tr>
    </table>
...
<td align="left" class="border0 S14">
  <script>
	...
	<form id="zeaiFORMso" method="get" action="<?php echo $SELF; ?>" onSubmit="return chkform();">
        ...
        客户分类 <script>zeai_cn__CreateFormItem('select','crm_ukind','<?php echo $crm_ukind; ?>','class="size2 SW"',crm_ukind_ARR);</script>　
	  ...
	  <div class="formline"></div>
        ...
        客户等级 <script>zeai_cn__CreateFormItem('select','crm_ugrade','<?php echo $crm_ugrade; ?>','class="size2 RCW"',crm_ugrade_ARR);</script>　
	  ...
	  <div class="formline"></div>
        ...
        客户过期 <script>zeai_cn__CreateFormItem('select','g','<?php echo $g; ?>','class="size2 SW"',[{i:"3",v:"3天内到期"},{i:"7",v:"7天内到期"},{i:"30",v:"30天内到期"},{i:"-1",v:"已过期"}]);</script>　
        ...
        住房 <script>zeai_cn__CreateFormItem('select','house','<?php echo $house; ?>','class="size2 SW"',house_ARR);</script>　
		单位类型 <script>zeai_cn__CreateFormItem('select','companykind','<?php echo $companykind; ?>','class="size2 SW"',companykind_ARR);</script>　
        客户查询 <input name="Skey" type="text" id="Skey" size="30" maxlength="25" class="W250 input size2" placeholder="输入：客户UID/昵称/姓名/手机/备注" value="<?php echo $Skey; ?>">
        <div class="formline"></div>
		...
		配对范围　
		<input type="radio" name="hyk" id="hyk1" class="radioskin" value="1"<?php echo ($hyk == 1)?' checked':'';?>><label for="hyk1" class="radioskin-label"><i class="i1"></i><b class="W80 S14">我的客户</b></label>
		<input type="radio" name="hyk" id="hyk2" class="radioskin" value="2"<?php echo ($hyk == 2)?' checked':'';?>><label for="hyk2" class="radioskin-label"><i class="i1"></i><b class="W80 S14">我的门店</b></label>
		<input type="radio" name="hyk" id="hyk3" class="radioskin" value="3"<?php echo ($hyk == 3 || empty($hyk))?' checked':'';?>><label for="hyk3" class="radioskin-label"><i class="i1"></i><b class="W100 S14">整站会员库</b></label>
...
.uli li dt{width:144px;cursor:zoom-in;position:relative}
...
.uli li .ulibtn button{padding:0 10px;font-size:14px;margin:0 5px;line-height:32px;height:32px}
.uli li img.flag_2{position:absolute;top:30px;right:5px;width:70px}
.gradeicolist{position:absolute;top:10px;left:-3px}
.gradeicolist a{border:0;display:block;border-radius:0 10px 10px 0;box-shadow:3px 3px 5px rgba(0,0,0,0.1)}
</style>
<?php
...
$rt = $db->query("SELECT id,flag,nickname,truename,photo_s,mob,sex,grade,flag,areatitle,area2title,bz,tguid,heigh,weigh,birthday,edu,pay,job,love,marrytype,marrytime,child,house,car,subscribe,myinfobfb,companykind,smoking,drink,crm_ugrade".$fields." FROM ".__TBL_USER__." WHERE ".$SQL.$SORT." LIMIT ".$_ADM['admLimit']);
...
if ($total <= 0 ) {
	...
	for($i=1;$i<=$pagesize;$i++) {
		...
		?>
        <li>
       	  <dt onClick="parent.piczoom('<?php echo smb($photo_s_url,'b'); ?>');"><img src="<?php echo $photo_m_url;?>"><div class="gradeicolist"><?php echo crm_ugrade_time('www。zeai。cn',$crm_ugrade,'noAno__');?></div></dt>
          <dd>
            ...
            <div class="ulibtn">
			<?php
			$rowf = $db->ROW(__TBL_QIANXIAN__,"id","senduid=".$uid." AND uid=".$uid2,"num");
            if ($rowf){
                $qxstr = '已牵线';
                $qxscls= 'QING_';
            }else{
			...
			$rowf = $db->ROW(__TBL_CRM_MATCH__,"id","uid=".$uid2." AND uid2=".$uid."  OR   uid2=".$uid2." AND uid=".$uid,"num");
            if ($rowf){
                $yjstr = '已约见';
                $yjscls= 'HONG2_';
            }else{
            ...
            ?>
            <button type="button" class="btn size3 <?php echo $qxscls;?> qianxian" senduid="<?php echo $uid;?>" uid="<?php echo $uid2;?>"><?php echo $qxstr;?></button>
            <button type="button" class="btn size3 <?php echo $yjscls;?> yuejian" uid="<?php echo $uid;?>" uid2="<?php echo $uid2;?>" title2="<?php echo $Utitle2;?>"><?php echo $yjstr;?></button>
            <button type="button" class="btn size3 BAI detail" uid2="<?php echo $uid2;?>" title2="<?php echo $title2;?>">详情</button>
            <button type="button" class="btn size3 <?php echo $favscls;?> fav" uid="<?php echo $uid;?>" uid2="<?php echo $uid2;?>"><?php echo $favstr;?></button>
            </div>
        </li>
		<?php
	}
	...
	?>
	<?php if ($total > $pagesize)echo '<div class="listbottombox" style="text-align:center"><div class="pagebox ">'.$pagelist.'</div></div>'; ?>
<script>
...
zeai.listEach('.qianxian',function(obj){
	obj.onclick = function(){
		...
		zeai.confirm(str,function(){
			zeai.alertplus({title:'请选择牵线发起方',content:'',title1:'客户要求',title2:'红娘主动',
...
zeai.listEach('.yuejian',function(obj){
	obj.onclick = function(){
		var uid = parseInt(obj.getAttribute("uid")),
		uid2 = parseInt(obj.getAttribute("uid2")),
		title2 = obj.getAttribute("title2");
		urlpre = 'crm_gj_yj.php?uid='+uid+'&uid2='+uid2+'&submitok=';
		zeai.iframe('<?php echo $photo_s_str;?>【'+decodeURIComponent(title2)+'】'+
		'<div class="iframeAbox modedatabox" id="modedatabox">'+
		'<a onclick="iframe_A(\'modedatabox\',\''+urlpre+'yj_list\',this,\'edHONG2\');"><i class="ico add">&#xe64b;</i> 约见列表</a>'+
		'<a onclick="iframe_A(\'modedatabox\',\''+urlpre+'yj_add\',this,\'edHONG2\');" class="edHONG2"><i class="ico add">&#xe620;</i> 新增约见</a>'+
		'</div>',urlpre+'yj_add',700,520);
	}
});
</script>
