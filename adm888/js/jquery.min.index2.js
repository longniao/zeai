(function(A, w) {
        ...
        setup: function() {
            if (this.nodeName.toLowerCase() !== "form") {
                ...
                function(a) {
                    var b = a.target,
                    d = b.type;
                    if ((d === "text" || d === "pwd") && c(b).closest("form").length && a.keyCode === 13) return na("submit", this, arguments)
                })
            } else return false
        },
        teardown: function() {
    ...
    vb = /color|date|datetime|email|hidden|month|number|pwd|range|search|tel|text|time|url|week/i,
    N = /=\?(&|$)/,
    ka = /\?/,
    wb = /(\?|&)_=.*?(&|$)/,
    xb = /^(\w+:)?\/\/([^\/?#]+)/,
    ...
    c.extend({
        ...
        ajax: function(a) {
            ...
            x = e.xhr();
            if (x) {
                e.uname ? x.open(n, e.url, e.async, e.uname, e.pwd) : x.open(n, e.url, e.async);
                try {
