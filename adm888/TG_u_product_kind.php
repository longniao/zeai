...
if ($submitok == 'ajax_addupdate' || $submitok == 'ajax_modupdate'){
	...
	AddLog('【商家/机构】->商品分类发布修改【商家id:'.$tg_uid.'】');
}
switch ($submitok){
	case "ajax_addupdate":
		$db->query("INSERT INTO ".__TBL_TG_PRODUCT_KIND__." (tg_uid,title,px) VALUES ($tg_uid,'$title',".ADDTIME.")");	
		json_exit(array('flag'=>1));
	break;
	case "ajax_modupdate":
		if(!ifint($id))json_exit(array('flag'=>0,'msg'=>'forbidden'));
		$db->query("UPDATE ".__TBL_TG_PRODUCT_KIND__." SET title='$title' WHERE id=".$id);
		$db->query("UPDATE ".__TBL_TG_PRODUCT__." SET kindtitle='$title' WHERE kind=".$id);
		json_exit(array('flag'=>1));
	break;
	case "ajax_delupdate":
		if(!ifint($id))json_exit(array('flag'=>0,'msg'=>'forbidden'));
		//$db->query("DELETE FROM ".__TBL_TG_PRODUCT__." WHERE kind=".$id);
		$db->query("DELETE FROM ".__TBL_TG_PRODUCT_KIND__." WHERE id=".$id);
...
<?php
	...
	$row = $db->ROW(__TBL_TG_PRODUCT_KIND__,"title","tg_uid=".$tg_uid." AND id=".$id,"name");
	if ($row){
...
<?php }else{?>
<div class="navbox">
	<a href="TG_u_mod.php?tg_uid=<?php echo $tg_uid; ?>&submitok=mod&k=<?php echo $k;?>"><?php echo $_SHOP['title'].'【ID:'.$tg_uid.'】';?>信息</a>
    <a href="TG_u_product.php?tg_uid=<?php echo $tg_uid; ?>&k=2"><?php echo $cname.'【ID:'.$tg_uid.'】';?>商品管理</a>
    <a href="TG_u_product_kind.php?tg_uid=<?php echo $tg_uid; ?>&k=2" class="ed"><?php echo $TG_set['tgytitle'].'【ID:'.$tg_uid.'】';?>商品分类<?php echo '<b>'.$db->COUNT(__TBL_TG_PRODUCT_KIND__,"tg_uid=".$tg_uid).'</b>';?></a>
    <div class="clear"></div>
</div>
<div class="fixedblank"></div>
	...
	<?php
	$rt = $db->query("SELECT id,title FROM ".__TBL_TG_PRODUCT_KIND__." WHERE tg_uid=".$tg_uid." ORDER BY px DESC,id DESC");
	$total = $db->num_rows($rt);
	if ($total <= 0 ) {
		if ($submitok !== "add")echo "<div class='nodataico'><i></i>暂无分类<br><a class='btn size2 HUANG' onClick=\"zeai.iframe('新增分类','".SELF."?submitok=add&tg_uid=".$tg_uid."',500,300)\">新增分类</a></div>";
	} else {
		$page_skin = 2;$pagesize=20;require_once ZEAI.'sub/page.php';
	?>
	<table class="table0 W98_ Mbottom10 Mtop10">
	  <tr>
		<td width="120" align="left"><button type="button" class="btn tips" onClick="zeai.iframe('新增分类','<?php echo SELF;?>?submitok=add&tg_uid=<?php echo $tg_uid;?>',500,300)" ><i class="ico addico">&#xe620;</i>新增分类</button></td>
		<td align="left">&nbsp;</td>
	  </tr>
	</table>
	...
	for($i=1;$i<=$pagesize;$i++) {
		...
		$wznum = $db->COUNT(__TBL_TG_PRODUCT__,"kind=".$id." AND tg_uid=".$tg_uid);
	?>
	<tr>
	<td width="60" height="40" align="center"><?php echo $id;?></td>
    <td width="60" height="40" align="center"><a href="<?php echo SELF."?id=".$id; ?>&submitok=ding&tg_uid=<?php echo $tg_uid;?>" class="topico" title="置顶"></a></td>
	<td width="150" class="S14"><?php echo $title;?></td>
	<td width="120" class="S14"><a href="TG_u_product.php?kind=<?php echo $id;?>&tg_uid=<?php echo $tg_uid;?>" class="aHUI"><?php echo($wznum>0)?'<font class="Cf00 S14">'.$wznum.'</font>':0;?></a></td>
	<td align="center">&nbsp;</td>
	<td width="80" class="center"><a value="<?php echo $id;?>" class="editico" title="<?php echo $title;?>" onClick="zeai.iframe('修改【<?php echo $title;?>】分类','<?php echo SELF;?>?submitok=mod&id=<?php echo $id;?>&tg_uid=<?php echo $tg_uid;?>&k=2',500,300)"></a></td>
	<td width="80" class="center"><a value="<?php echo $id; ?>" wznum="<?php echo $wznum;?>" class="delico" title="<?php echo $title;?>"></a></td>
	</tr>
	<?php } ?>
	<?php if ($total > $pagesize){?>
	<tfoot><tr>
	<td colspan="7" class="Pleft10"><?php echo '<div class="pagebox FR">'.$pagelist.'</div>'; ?></td>
	</tr></tfoot>
	<?php } ?>
	</table>
	<?php } ?>
...
<?php if ($submitok == "add" || $submitok == "mod") {?>
	save.onclick = function(){
		zeai.ajax({url:'TG_u_product_kind'+zeai.extname,form:ZEAIFORM},function(e){var rs=zeai.jsoneval(e);
			if (rs.flag == 1){
...
<?php }else{ ?>
	zeai.listEach('.delico',function(obj){
		...
		var tips = (wznum>0)?'当前分类包含 '+wznum+' 条信息':'';
		obj.onclick = function(){
			zeai.confirm('<font color="red">请慎重！</font><br>'+tips+'<br>真的要删除【'+title+'】么？<br>删除后不可恢复',function(){
				zeai.ajax('TG_u_product_kind'+zeai.ajxext+'submitok=ajax_delupdate&id='+id+"&tg_uid=<?php echo $tg_uid;?>",function(e){var rs=zeai.jsoneval(e);
