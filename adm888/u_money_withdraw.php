...
class WxPay_class{
	...
	public function reback_err($errid,$err_money=100){
		switch($errid){
		   ...
		   case "CA_ERROR": return "证书出错，请登录微信支付商户平台下载API证书传到后台cert目录下"; break;
		}	
	}
}
?>
...
.table {width: 90%;	min-width: 400px;}
...
if($submitok=="gopayfor"){
	if ( !ifint($id) || !ifint($uid))alert("forbidden","-1");
	$rt = $db->query("SELECT uid,flag,orderid,money_list_id,paymoney FROM ".__TBL_PAY__." WHERE kind=-1 AND id='$id'");
	if ($db->num_rows($rt)) {
		$rows       = $db->fetch_array($rt);
		$uid        = $rows['uid'];
		$row = $db->NUM($uid,"truename,nickname,mob,openid,photo_s,money");
		if ($row){
		...
		<?php
			...
			if($wxpay_arr['return_code']=="SUCCESS" && $wxpay_arr['result_code']=="SUCCESS"){
				...
				*/
				AddLog('【提现打款】用户【'.$data_nickname.'（uid:'.$uid.'）】->提现打款成功，金额：￥'.$pay_money);
			?>
				<table width="500" border="0" cellspacing="10" cellpadding="0" style="margin-top:50px;" align="center">
				  <tr>
					...
					<td align="left" valign="middle" nowrap="nowrap" style="color:#090; font-family:'微软雅黑'; font-size:24px;">付款成功！</td>
				  </tr>
				  <tr>
	...
	$rt = $db->query("SELECT id,uid,paymoney,flag,orderid,addtime FROM ".__TBL_PAY__." WHERE id='$id'");
	if ($db->num_rows($rt)){
		$rows = $db->fetch_array($rt,'name');
		$uid = $rows['uid'];
		$row = $db->NUM($uid,"truename,nickname,mob,openid,photo_s");
		if ($row){
			$data_truename = dataIO($row[0],'out');
			$data_nickname = dataIO($row[1],'out');
			$data_mob      = dataIO($row[2],'out');
...
<!--            <tr>
            ...
            <tr>
            <td class="tdL">收款人</td>
            <td class="tdR"><?php echo $nickname.$truename;?>　UID：<?php echo $uid;?></td>
            </tr>
            <tr>
            <td class="tdL">手机号码</td>
            <td class="tdR"><?php echo $mob;?></td>
            </tr>
            <tr>
            <td class="tdL">微信openid</td>
            <td class="tdR"><?php echo empty($data_openid)?'<font class="C00f">openid为空，请改为人工转账【手动打款】</font>':$data_openid.'<div class="Cf00 S12">打款成功后，此用户微信钱包将收到此款项</div>';?></td>
            </tr>
            <tr>
