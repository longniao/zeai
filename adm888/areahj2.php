...
switch ($submitok){
	case "ajax_addupdate":
		$row = $db->NUM('zeai.area',"title","id=".$id." AND fid=".$fid,__TBL_AREAHJ2__);
		if ($row)json_exit(array('flag'=>0,'msg'=>'地区【'.dataIO($row[0],'out').'】　ID：<font class=Cfc0>'.$id.'</font>　出现重复，请重试','focus'=>'id'));
		$db->query("INSERT INTO ".__TBL_AREAHJ2__." (id,title,fid,px) VALUES ($id,'$title',$fid,".ADDTIME.")");
		json_exit(array('flag'=>1));
	break;
	case "ajax_modupdate":
		$db->query("UPDATE ".__TBL_AREAHJ2__." SET title='$title' WHERE id=".$id);
		json_exit(array('flag'=>1));
	break;
	case "ajax_delupdate":
		if(!ifint($id))json_exit(array('flag'=>0,'msg'=>'forbidden'));
		if ($db->COUNT(__TBL_AREAHJ2__,"fid=".$fid) <= 1)json_exit(array('flag'=>0,'msg'=>'亲，不能删光啊，至少要留一个啊'));
		$db->query("DELETE FROM ".__TBL_AREAHJ3__." WHERE fid=".$id);
		$db->query("DELETE FROM ".__TBL_AREAHJ2__." WHERE id=".$id);
		json_exit(array('flag'=>1,'msg'=>'删除成功'));
	break;
	case "ding":
		if (!ifint($id))alert_adm_parent('forbidden','back');
		$db->query("UPDATE ".__TBL_AREAHJ2__." SET px=".ADDTIME." WHERE id=".$id);
		header("Location: $SELF?fid=$fid");
	break;
	case "sort_1"://一级上移
		if (!ifint($id))alert_adm_parent('forbidden','back');
		$row = $db->NUM('zeai.area',"px","id=".$id,__TBL_AREAHJ2__);
		if ($row){$px=$row[0];}else{exit;}
		$row = $db->NUM('zeai.area',"id,px","px>".$px." AND fid=".$fid." ORDER BY px LIMIT 1",__TBL_AREAHJ2__);
		if ($row){
			$tmp_id = $row[0];
			$tmp_px = $row[1];
			$db->query("UPDATE ".__TBL_AREAHJ2__." SET px=".$tmp_px." WHERE id=".$id);
			$db->query("UPDATE ".__TBL_AREAHJ2__." SET px=".$px." WHERE id=".$tmp_id);
		}
		header("Location: $SELF?fid=$fid");
	break;
	case "sort_0"://一级下移
		...
		$row = $db->NUM('zeai.area',"px","id=".$id,__TBL_AREAHJ2__);
		if ($row){$px=$row[0];}else{exit;}
		$row = $db->NUM('zeai.area',"id,px","px<".$px." AND fid=".$fid." ORDER BY px DESC LIMIT 1",__TBL_AREAHJ2__);
		if ($row){
			$tmp_id = $row[0];
			$tmp_px = $row[1];
			$db->query("UPDATE ".__TBL_AREAHJ2__." SET px=".$tmp_px." WHERE id=".$id);
...
$row = $db->NUM('zeai.area',"title","id=".$fid,__TBL_AREAHJ1__);
...
<?php }else{?>
	<div class="navbox">
		<a href="areahj1.php">一级地区管理</a>
		<a class="ed" href="#top">【<?php echo $title1;?>】下属二级地区管理<b><?php echo $db->COUNT(__TBL_AREAHJ2__,"fid=".$fid);?></b></a>
	</div>
	<div class="fixedblank"></div>
	<?php
	$rt = $db->query("SELECT id,title FROM ".__TBL_AREAHJ2__." WHERE fid=".$fid." ORDER BY px DESC");
	...
	<tr>
		...
		<td>
			<?php
			$rt2=$db->query("SELECT title FROM ".__TBL_AREAHJ3__." WHERE fid=".$id." ORDER BY px DESC");
			...
			if ($total2 > 0) {
				for($i2=1;$i2<=$total2;$i2++) {
					...
					if(!$rows2) break;
					$title2 = $rows2[0];
					echo '<a class="li" href="areahj3.php?fid='.$id.'" title="管理三级">'.$title2.'</a>';
				}
			}
			?>
            <a class='btn size1' href="areahj3.php?fid=<?php echo $id;?>" >管理三级</a>
		</td>
		<td width="20">&nbsp;</td>
...
<?php if ($submitok == "add") {?>
	save.onclick = function(){
		zeai.ajax({url:'areahj2'+zeai.extname,form:ZEAIFORM},function(e){var rs=zeai.jsoneval(e);
			if (rs.flag == 1){
...
<?php }else{ ?>
	zeai.listEach('.modbtn',function(obj){
		...
		obj.onclick = function(){
			var V = C.value;
			zeai.ajax('areahj2'+zeai.ajxext+'submitok=ajax_modupdate&id='+id+'&title='+V+'&fid=<?php echo $fid;?>',function(e){var rs=zeai.jsoneval(e);
	...
	zeai.listEach('.del',function(obj){
		...
		obj.onclick = function(){
			zeai.confirm('将同步删除【'+title+'】下属二级和三级地区，真的要删除么？',function(){
				zeai.ajax('areahj2'+zeai.ajxext+'submitok=ajax_delupdate&id='+id+'&fid=<?php echo $fid;?>',function(e){var rs=zeai.jsoneval(e);
					if (rs.flag == 1){location.reload(true);}else{zeai.msg(rs.msg);}
				});
			});
		}
