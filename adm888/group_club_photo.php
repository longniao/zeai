...
if ($submitok == "批量删除"){
	...
	if (count($tmeplist) >= 1){
		foreach ($tmeplist as $value) {
			if(!ifint($value))alert_adm("forbidden","-1");
			$rt = $db->query("SELECT clubid,path_s,path_b,ifmain FROM ".__TBL_GROUP_CLUB_PHOTO__." WHERE id=".$value);
			if($db->num_rows($rt)){
			...
			$db->query("DELETE FROM ".__TBL_GROUP_CLUB_PHOTO__." WHERE id=".$value);
		}
...
<div class="navbox">
    ...
    <a href="group_club_photo.php" class="ed">活动照片<?php echo '<b>'.$db->COUNT(__TBL_GROUP_CLUB_PHOTO__).'</b>';?></a>
    <a href="group_club_bbs.php">活动评论</a>
<div class="clear"></div></div>
<div class="fixedblank"></div>
<?php 
$rt=$db->query("SELECT * FROM ".__TBL_GROUP_CLUB_PHOTO__." ORDER BY id DESC LIMIT ".$_ZEAI['limit']);
...
<a href="###" onClick="parent.piczoom('<?php echo $_ZEAI['up2']."/".$rows['path_b']; ?>')" ><img src=<?php echo $_ZEAI['up2']."/";?>/<?php echo  $rows['path_s']; ?> alt="放大照片" border="0"></a></div></td>
      ...
      <tr>
        <td colspan="2" <?php echo  $bg; ?>>来自活动：<b><a href="<?php echo Href('group_party',$rows['clubid']); ?>" target="_blank"><img src="images/zoom.gif" hspace="5" border="0" align="absmiddle" ><font color="#FF0000"><u><?php echo $rows['clubid']; ?></u></font></a></b></td>
      <tr>
        <td width="60" height="5" align="right" valign="bottom" style="color:#ff0000" title="主照片"><?php if ($rows['ifmain'] == 1){echo '★★★';}?></td>
        <td width="90" align="right"><input type=checkbox name=list[] value="<?php echo $rows['id']; ?>" id="chk<?php echo $rows['id']; ?>" style="border:0px;vertical-align: middle;"><label for="chk<?php echo $rows['id']; ?>"><font color="#999999">选择</font></label></td>
      </tr>
    </table></td>
    <?php if ($i % 6 == 0) {?>
  </tr>
