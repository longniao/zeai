...
/*************************************************/
var nulltext = '请选择';
function LevelMenu3(clientstr,selectext,fobj) {
	...
	var objid3    = s[2];
	var nulltext  = s[3];
	build_select(objid1,nulltext,selectext,fobj);
	//document.write(' ');
	fobj.append(' ');
	var selectext2= "onchange=levelmenu3_chanage(this.value,'"+clientstr+"')";
	build_select(objid2,nulltext,selectext+" "+selectext2,fobj);
	//document.write(' ');
	fobj.append(' ');
	build_select(objid3,nulltext,selectext,fobj);
	create1(clientstr);
}
function build_select(objid,nulltext,selectext,fobj) {
...
function create_checkbox_arr_list(objid,defvalue,selectext,ARR){
	...
	for(var k=0;k<ARR.length;k++){
		...
		obj.onclick = function (){if (zeai.form.ifcheckbox(objid+'[]') > 3){zeai.alert('最多选择 3 项');return false;}};
...
function create_select_arr_list(objid,defvalue,selectext,ARR,fobj){
	var ARR       = arguments[3] ? arguments[3]:'';
	var selectext = arguments[2] ? arguments[2]:'class="SW"';
	ARR = (zeai.empty(ARR))?eval(objid+'_ARR'):ARR;
	build_select(objid,nulltext,selectext,fobj);
...
function zeai_cn__CreateFormItem_ajax(type,objid,defvalue,selectext,ARR,fobj){
	switch (type) {
		case    'radio':create_radio_arr_list(objid,defvalue,selectext,ARR,fobj);break;
		case 'checkbox':create_checkbox_arr_list(objid,defvalue,selectext,ARR,fobj);break;
		case   'select':create_select_arr_list(objid,defvalue,selectext,ARR,fobj);break;
		case     'text':create_text_arr_list(objid,defvalue,selectext,fobj);break;
		default:break;
	}
}
