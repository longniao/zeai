...
class Common_util_pub
{
	...
	function postXmlSSLCurl($xml,$url,$second=30)
	{
		...
		curl_setopt($ch,CURLOPT_SSLCERT,$yp_root.WxPayConf_pub::SSLCERT_PATH);
		//默认格式为PEM，可以注释
		curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
		curl_setopt($ch,CURLOPT_SSLKEY,$yp_root.WxPayConf_pub::SSLKEY_PATH);
		//post提交方式
		curl_setopt($ch,CURLOPT_POST, true);
		curl_setopt($ch,CURLOPT_POSTFIELDS,$xml);
		$data = curl_exec($ch);
	...
	function createXml(){
		try{
			...
			if($this->parameters["out_trade_no"] == null){
		   	...
		   	$this->parameters["spbill_create_ip"] = getip();//终端ip	 
			   
		    $this->parameters["nonce_str"] = $this->createNoncestr();//随机字符串
		    $this->parameters["sign"] = $this->getSign($this->parameters);//签名
			
			//gyl_debug('$this->parameters='.http_build_query($this->parameters,'','&'));
