/******************************************
WWW.ESYYW.COM 作者: 李林　QQ:721688068 (supdes)
...
var zeai = {
	...
	*/
	...
	back:function (){document.referrer === ''?window.location.href = HOST:window.history.go(-1);},
					...
					function photo_up(){
						...
						zeai.ajax({"url":json.url,"ajaxLoading":json.ajaxLoading,"data":postjson},function(e){var rs=zeai.jsoneval(e);
							if (rs.flag == 1){
								...
								picNull();if(typeof(json.fn)=="function")json.fn(e);
								//zeai.msg('【'+FILES[j]['name']+'】上传出错，请联系原作者QQ：721688068');
							}
						});
					}
					photo_up();
	...
	confirm:function(title,fn){
		...
		//if (this.empty(o('confirm_mask'))){
			...
			confirm_box.style.top  = parseInt((this.bodyH() - cH)/2-100) + 'px';
		//}else{
